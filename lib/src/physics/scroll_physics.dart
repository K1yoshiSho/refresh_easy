...
class _ERScrollPhysics extends ClampingScrollPhysics {
  _ERScrollPhysics({
    ...
    _headerSimulationCreationState = ValueNotifier(_BallisticSimulationCreationState(
      mode: headerNotifier.mode,
      offset: headerNotifier.offset,
      actualTriggerOffset: headerNotifier.actualTriggerOffset,
    ));
    _footerSimulationCreationState = ValueNotifier(_BallisticSimulationCreationState(
  ...
  late final ValueNotifier<_BallisticSimulationCreationState> _headerSimulationCreationState;
  late final ValueNotifier<_BallisticSimulationCreationState> _footerSimulationCreationState;

  /// Get the current [SpringDescription] to be used.
  @override
  physics.SpringDescription get spring {
    if (headerNotifier.outOfRange) {
      if (headerNotifier._mode == IndicatorMode.ready && headerNotifier.readySpringBuilder != null) {
    ...
    if (footerNotifier.outOfRange) {
      if (footerNotifier._mode == IndicatorMode.ready && footerNotifier.readySpringBuilder != null) {
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    ...
    if (headerNotifier.secondaryLocked) {
      // Header secondary
      pixels = headerNotifier.secondaryDimension + (headerNotifier.secondaryDimension + position.pixels);
      minScrollExtent = 0;
    ...
    if (footerNotifier.secondaryLocked) {
      // Footer secondary
      pixels = position.pixels - footerNotifier.secondaryDimension - position.maxScrollExtent;
      minScrollExtent = 0;
    ...
    return direction * _applyFriction(overscrollPast, offset.abs(), 0);
  }
  ...
  double applyBoundaryConditions(ScrollMetrics position, double value) {
    if (headerNotifier._axis != position.axis || headerNotifier._axisDirection != position.axisDirection) {
      headerNotifier._axis = position.axis;
      headerNotifier._axisDirection = position.axisDirection;
    }
    if (footerNotifier._axis != position.axis || footerNotifier._axisDirection != position.axisDirection) {
    ...
    if (headerNotifier.clamping == true) {
      if (value < position.minScrollExtent &&
          (position.minScrollExtent < position.pixels ||
              // NestedScrollView
              (!userOffsetNotifier.value && position.minScrollExtent == position.pixels))) {
        // hit top edge
        _updateIndicatorOffset(position, 0, value);
        return value - position.minScrollExtent;
      } else if (value < position.pixels && position.pixels <= position.minScrollExtent) {
        // underscroll
        bounds = value - position.pixels;
      } else if (headerNotifier._offset > 0 && !(headerNotifier.modeLocked || headerNotifier.secondaryLocked)) {
      ...
      // Maximum overscroll offset
      if (headerNotifier.actualMaxOverOffset != double.infinity && value < -headerNotifier.actualMaxOverOffset) {
        _updateIndicatorOffset(position, -headerNotifier.actualMaxOverOffset, value);
        return (value + headerNotifier.actualMaxOverOffset) - position.minScrollExtent;
      }
      // hit top over
      ...
      if (headerNotifier._isSupportAxis &&
          (!headerNotifier.infiniteHitOver || (!headerNotifier.hitOver && headerNotifier.modeLocked)) &&
          (headerNotifier._canProcess || headerNotifier.noMoreLocked) &&
          (value + headerNotifier.actualTriggerOffset) < position.minScrollExtent &&
          (position.minScrollExtent < (position.pixels + headerNotifier.actualTriggerOffset) ||
              // NestedScrollView
              (!userOffsetNotifier.value &&
                  position.minScrollExtent == (position.pixels + headerNotifier.actualTriggerOffset)))) {
        _updateIndicatorOffset(position, -headerNotifier.actualTriggerOffset, value);
        return (value + headerNotifier.actualTriggerOffset) - position.minScrollExtent;
      }
      // Stop spring rebound.
      if (headerNotifier._releaseOffset > 0 &&
        ...
        _updateIndicatorOffset(position, -headerNotifier.actualTriggerOffset, value);
        return headerNotifier.actualTriggerOffset + value - position.minScrollExtent;
      }
      // Cannot over the secondary.
      if (headerNotifier.hasSecondary) {
        if (value < position.pixels &&
            position.pixels <= position.minScrollExtent - headerNotifier.secondaryDimension) {
          // underscroll secondary
          bounds = value - position.pixels;
        } else if (value + headerNotifier.secondaryDimension < position.minScrollExtent &&
            position.minScrollExtent < position.pixels + headerNotifier.secondaryDimension) {
          // hit top secondary
          _updateIndicatorOffset(position, -headerNotifier.secondaryDimension, value);
          return value + headerNotifier.secondaryDimension - position.minScrollExtent;
        }
      }
    }
    ...
    if (footerNotifier.clamping == true) {
      if ((position.pixels < position.maxScrollExtent ||
              // NestedScrollView
              (!userOffsetNotifier.value && position.pixels == position.maxScrollExtent)) &&
        ...
        // overscroll
        bounds = value - position.pixels;
      } else if (footerNotifier._offset > 0 && !(footerNotifier.modeLocked || footerNotifier.secondaryLocked)) {
      ...
      if (footerNotifier.actualMaxOverOffset != double.infinity &&
          position.maxScrollExtent < value - footerNotifier.actualMaxOverOffset) {
        _updateIndicatorOffset(position, position.maxScrollExtent + footerNotifier.actualMaxOverOffset, value);
        return (value - footerNotifier.actualMaxOverOffset) - position.maxScrollExtent;
      }
      // hit bottom over
      ...
      if (footerNotifier._isSupportAxis &&
          !(footerNotifier.infiniteOffset != null && position.maxScrollExtent <= position.minScrollExtent) &&
          (!footerNotifier.infiniteHitOver || !footerNotifier.hitOver && footerNotifier.modeLocked) &&
          (footerNotifier._canProcess || footerNotifier.noMoreLocked) &&
          ((position.pixels - footerNotifier.actualTriggerOffset) < position.maxScrollExtent ||
              // NestedScrollView
              (!userOffsetNotifier.value &&
                  (position.pixels - footerNotifier.actualTriggerOffset) == position.maxScrollExtent)) &&
          position.maxScrollExtent < (value - footerNotifier.actualTriggerOffset)) {
        _updateIndicatorOffset(position, position.maxScrollExtent + footerNotifier.actualTriggerOffset, value);
        return (value - footerNotifier.actualTriggerOffset) - position.maxScrollExtent;
      }
      // Stop spring rebound.
      if (footerNotifier._releaseOffset > 0 &&
          footerNotifier._mode == IndicatorMode.ready &&
          !footerNotifier._indicator.springRebound &&
          value < position.maxScrollExtent + footerNotifier.actualTriggerOffset) {
        _updateIndicatorOffset(position, position.maxScrollExtent + footerNotifier.actualTriggerOffset, value);
        return (value - footerNotifier.actualTriggerOffset) - position.maxScrollExtent;
      }
      // Cannot over the secondary.
      if (footerNotifier.hasSecondary) {
        if (position.maxScrollExtent + footerNotifier.secondaryDimension <= position.pixels &&
            position.pixels < value) {
          // overscroll
          bounds = value - position.pixels;
        } else if (position.pixels - footerNotifier.secondaryDimension < position.maxScrollExtent &&
            position.maxScrollExtent < value - footerNotifier.secondaryDimension) {
          // hit bottom edge
          _updateIndicatorOffset(position, position.maxScrollExtent + footerNotifier.secondaryDimension, value);
          return value - footerNotifier.secondaryDimension - position.maxScrollExtent;
        }
      }
    }
    // Update offset
  ...
  void _updateIndicatorOffset(ScrollMetrics position, double offset, double value) {
  ...
  Tolerance getTolerance(ScrollMetrics metrics) {
    try {
      ...
      return Tolerance(
        velocity: 1.0 / (0.050 * metrics.viewportDimension),
        distance: 1.0 / metrics.viewportDimension,
      );
    }
  }
  ...
  Simulation? createBallisticSimulation(ScrollMetrics position, double velocity) {
    ...
    final oldMaxScrollExtent = headerNotifier._lastMaxScrollExtent ?? position.maxScrollExtent;
    // Simulation update.
    ...
    bool hSecondary = !headerNotifier.clamping &&
        (headerNotifier._mode == IndicatorMode.secondaryReady || headerNotifier._mode == IndicatorMode.secondaryOpen);
    bool fSecondary = !headerNotifier.clamping &&
        (footerNotifier._mode == IndicatorMode.secondaryReady || footerNotifier._mode == IndicatorMode.secondaryOpen);
    bool secondary = hSecondary || fSecondary;
    if (velocity.abs() >= tolerance.velocity ||
        (oldMaxScrollExtent != position.maxScrollExtent && position.maxScrollExtent != 0) ||
...
class _BallisticSimulationCreationState {
  ...
  bool needCreation(_BallisticSimulationCreationState newState) {
    return mode != newState.mode ||
        ...
        (newState.mode == IndicatorMode.ready && newState.offset >= actualTriggerOffset);
  }
}
...
extension _ScrollMetricsExtension on ScrollMetrics {
  ...
  bool get isNestedOuter => this is ScrollPosition && (this as ScrollPosition).debugLabel == 'outer';

  // NestedScrollView inner.
  bool get isNestedInner => this is ScrollPosition && (this as ScrollPosition).debugLabel == 'inner';
}
